    ...
    public function store(BooksCreateRequest $request)
{
    ...
    if ($create_books) {
        return redirect('/admin/books')->with('success_message', 'Book created successfully');
    } else {
        // Handle error if book creation fails
        return redirect()->back()->with('error_message', 'Failed to create book');
    }
}
